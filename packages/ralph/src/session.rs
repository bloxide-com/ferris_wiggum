...
pub struct SessionManager {
    ...
    activity_channels: std::sync::Arc<
        tokio::sync::RwLock<
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        if !path.exists() {
            ...
            return Err(RalphError::Io(format!(
    ...
    pub async fn start_session(&self, id: &str) -> Result<Session, RalphError> {
        ...
        if prd.stories.is_empty() {
            ...
            return Err(RalphError::InvalidState(
                "Cannot start session with empty PRD. PRD must contain at least one story."
                    .to_string(),
            ));
        }

        tracing::info!(
            "Session {} has {} stories to process",
            id,
            prd.stories.len()
        );
        ...
        session.status = SessionStatus::Running {
            story_id: "initializing".to_string(),
        };
    ...
    pub async fn subscribe_to_activity(
        &self,
        session_id: &str,
    ) -> mpsc::UnboundedReceiver<ActivityEntry> {
    ...
    async fn run_loop(&self, mut session: Session) -> Result<(), RalphError> {
        ...
        tracing::warn!(
            "Session {} reached max iterations without completion",
            session.id
        );
    ...
    async fn run_iteration(&self, session: &mut Session) -> Result<IterationResult, RalphError> {
        ...
        let runner = CursorRunner::new(session.project_path.clone(), session.config.model.clone());
        ...
        if complete {
            tracing::info!(
                "Story completion signal received for session {}",
                session.id
            );
    ...
    async fn build_iteration_prompt(&self, session: &Session) -> Result<String, RalphError> {
        ...
        let guardrails = guardrail_manager
            .format_for_prompt()
            .await
